...
def update_teacher_availability_matrix(teacher_availability_matrix, best_chromosome):
    """Update teacher availability matrix based on best chromosome."""
...
def update_matrix_for_best(best_chromosome, teacher_availability_matrix, day_map, time_slot_map):
    """Update teacher availability matrix for the best timetable."""
    for day_name, section_dict in best_chromosome.items():
        ...
        for section_name, schedule_items in section_dict.items():
            for item in schedule_items:
                ...
                if (teacher_id in teacher_availability_matrix and
                    day_index < len(teacher_availability_matrix[teacher_id]) and
                    slot_index - 1 < len(teacher_availability_matrix[teacher_id][day_index])):
